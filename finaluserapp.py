    ...
    with cnxa.cursor() as cursor:
        if request.method == 'POST':
            ...
            if search_word == '':
                ...
                sqlque = "SELECT csp,cid,cname from cert where email='dharinath@virtusa.com' order by csp;".format(email)
                cursor.execute(sqlque)
                certificatetable = cursor.fetchall()
                cnxa.commit()
            else:    
                sqlque = "SELECT csp,cid,cname from cert WHERE csp = '{}';".format(search_word,email)
                cursor.execute(sqlque)
                certificatetable = cursor.fetchall()
                cnxa.commit()
    cursor.close()
...
def takequiz():
    ...
    with cnxa.cursor() as cursor:
        if request.method == 'POST':
            ...
            sqlquery = "select corr ect from {}quiz where quizname='{}'".format(iqcsp,iquizname)
            cursor.execute(sqlquery)
            qcorrecttuple = cursor.fetchall()

            qquestionlist=list(qquestiontuple)
            qoptionlist = []
            for optrow in qoptiontuple:
                qoptionlist.append(list(optrow))
            qcorrectlist = []
            for ans in qcorrecttuple:
                qcorrectlist.append(int(ans))
            cnxa.commit()
    cursor.close()
    return  render_template('takequiz.html', email=email, quizcsp=iqcsp, quizname=iquizname, question = qquestionlist, opt1 = json.dumps(qoptionlist[0]),opt2 = qoptionlist[1],opt3 = qoptionlist[2],opt4 = qoptionlist[3],opt5 = qoptionlist[4],opt6 = qoptionlist[5],opt7 = qoptionlist[6],opt8 = qoptionlist[7],opt9 = qoptionlist[8],opt10 = qoptionlist[9], ans = qcorrectlist)
    ...
    with cnxa.cursor() as cursor:
        if request.method == 'POST':
            ...
            try:
                ...
                try:
                    sqlque = "insert into {}score(quizname,email,totalscore,sec1,sec2,sec3,sec4,sec5,attempt) values(%s,%s,%d,%d,%d,%d,%d,%d,%s);".format(icsp)
                    vals = (iquizname,email,totalscore,sec1,sec2,sec3,sec4,sec5,"1")
                    cursor.execute(sqlque)
                    cnxa.commit()
                    #call the send report function 
                except:
